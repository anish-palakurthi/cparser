...
{
    unsigned n_params = get_riscv_jalr_attr_n_params(node);
    const arch_register_t *jalr_reg = arch_get_irn_register_in(node, 2);
    const char *jalr_reg_name = jalr_reg->name;

    // Generate CFI check
    be_emit_irprintf("\tlw t0, 0(%s)\n", jalr_reg_name);  // Load the first instruction of the target function
    be_emit_irprintf("\tsrli t0, t0, 16\n");              // Shift right to get the encoded arity
    be_emit_irprintf("\tandi t0, t0, 0xff\n");            // Mask to get only the arity bits
    be_emit_irprintf("\tli t1, %u\n", n_params);          // Load the expected number of parameters
    be_emit_irprintf("\tbne t0, t1, 1f\n");               // Branch to error if not equal
    
    // Original jalr instruction
    be_emit_irprintf("\tjalr\t%s\n", jalr_reg_name);
    be_emit_irprintf("\tj 2f\n");                         // Jump over error handling
