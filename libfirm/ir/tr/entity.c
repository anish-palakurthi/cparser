...
int get_entity_is_cfi_target(const ir_entity *ent)
{
    assert(is_Method_type(get_entity_type(ent)));
    return ent->attr.mtd_attr.is_cfi_target;
}

void set_entity_is_cfi_target(ir_entity *ent, int is_cfi_target)
{
    if (ent == NULL) {
        fprintf(stderr, "Warning: Attempted to set CFI target on NULL entity\n");
        return;
    }
    
    ir_type *type = get_entity_type(ent);
    if (type == NULL || !is_Method_type(type)) {
        fprintf(stderr, "Warning: Attempted to set CFI target on non-method entity\n");
        return;
    }
    ...
    // Debug output
    fprintf(stderr, "Set CFI target for entity %s: %d\n", get_entity_name(ent), is_cfi_target);
}
